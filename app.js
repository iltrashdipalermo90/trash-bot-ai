...
async function askGPT(msg) {
  try {
    const res = await openai.chat.completions.create({ model: 'gpt-3.5-turbo',
      messages: [{role:'system', content:'Rispondi sempre in italiano.'},{role:'user', content:msg}]});
    return res.choices[0].message.content.trim();
  } catch (e) { console.error(e); return 'Errore GPT'; }
}
...
async function startSession(id='default') {
  ...
  const sock = makeWASocket({version, printQRInTerminal:true, auth:state, logger});
  sock.ev.on('creds.update', saveCreds);
  sock.ev.on('connection.update', ({connection,lastDisconnect}) => {
    if (connection === 'close' && (lastDisconnect.error.output?.statusCode !== DisconnectReason.loggedOut)) startSession(id);
  });
  sock.ev.on('messages.upsert', async evt => {
    const msg = evt.messages?.[0]; if (!msg?.message) return;
    const jid = msg.key.remoteJid, text = msg.message.conversation || msg.message.extendedTextMessage?.text;
    if (!text || msg.key.fromMe) return;
    console.log(`[${jid}] ${text}`);
    if (text === '!stop') { disabled.add(jid); return sock.sendMessage(jid,{text:'Bot off'}); }
    if (text === '!start') { disabled.delete(jid); return sock.sendMessage(jid,{text:'Bot on'}); }
...
app.post('/start', (req,res) => {
  startSession(req.body.sessionId || 'default').then(()=>res.send({status:'ok'})).catch(e=>res.status(500).send({error:e.message}));
});
